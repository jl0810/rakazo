        ...
        const applyTool = async (
          ...
          if (name === "create_routine") {
            ...
            if (!routineName || !routinePrompt || !cron) {
              return finish({ error: "create_routine requires name, prompt, and cron." });
            }
            if (cron.split(/\s+/).length < 5) {
              return finish({
                error: `Invalid cron expression "${cron}". Use 5 fields, e.g. '0 9 * * *'.`,
              });
            }
          ...
          for await (const event of deps.runtime.run(
            {
              ...
              instructions: [
                ...
                rakazoCapabilityInstruction(tools),
            ...
            if (!event || event.type === undefined) continue;
            if (event.type === "text") {
